---
title: "Entre el mérito y la estructura social: Desigualdad educativa en el Gran Santiago"
subtitle: "Un análisis multinivel de factores individuales y territoriales"
author: "
...
2.  Analizar la influencia de factores comunales como la tasa de empleo, la oferta escolar y el IDH inciden en los niveles promedio de escolaridad en el Gran Santiago.
3.  Explorar cómo interactúan los factores familiares y comunales, evaluando si el impacto de las condiciones individuales -nivel socioeconomico- varía según el contexto territorial.
...
```{r descriptivos nivel 1, fig.cap="Figura 1.1: Estadisticos descriptivos de nivel 1", echo=FALSE, results='asis'}
library(stargazer)
...
```{r descriptivos, echo=FALSE, fig.cap="Figura 1.3: Estadisticos descriptivos de nivel 2", results='asis'}
library(stargazer)
...
$$
  ...
  \text{esc}_{ij} \;=\;
  \gamma_{00}
  + \gamma_{01}\,\text{prop\_empleo\_gmc}
  + \gamma_{02}\,\text{prop\_pob\_escn\_gmc}
  + \gamma_{03}\,\text{idh\_gmc}
  + u_{0j}
  + r_{ij}
  \end{aligned}
  \
...
$$
  ...
  \text{esc}_{ij} =\;&
    ...
    + \gamma_{03}\,\text{part\_social\_num} \\[4pt]
    &+ \gamma_{04}\,\text{prop\_empleo\_gmc}
    + \gamma_{05}\,\text{prop\_pob\_esc\_gmc}
    + \gamma_{06}\,\text{idh\_gmc} \\[4pt]
    &+ u_{0j}
    + u_{1j}\,\text{nse\_cmc}
    + r_{ij}
...
-   **Modelo 5 con interacción entre NSE y empleo:** El Modelo 5 incorpora una interacción entre NSE y la tasa comunal de empleo. Permite analizar si el impacto del nivel socioeconómico sobre la escolaridad cambia según las oportunidades laborales de cada comuna. Mantiene la pendiente aleatoria para NSE, profundizando en la influencia combinada del capital individual y contexto económico.\
    ...
    + \gamma_{05}\,\text{prop\_pob\_esc\_gmc}
    + \gamma_{06}\,\text{idh\_gmc} \\[4pt]
    &+ \gamma_{07}\bigl(\text{nse\_cmc}\,\times\,\text{prop\_empleo\_gmc}\bigr) \\[4pt]
    &+ u_{0j}
    + u_{1j}\,\text{nse\_cmc}
    + r_{ij}
      \end{aligned}
...
-   **Modelo 6 con interacción entre NSE y IDH:** El Modelo 6 introduce la interacción entre NSE y el índice de desarrollo humano comunal. Examina si el retorno educativo del NSE se modifica dependiendo del desarrollo social y económico del territorio. Esta especificación complementa el análisis, evidenciando cómo condiciones estructurales pueden amplificar o moderar desigualdades educativas entre comunas. $$
    ...
    + \gamma_{05}\,\text{prop\_pob\_esc\_gmc}
    + \gamma_{06}\,\text{idh\_gmc} \\[4pt]
    &+ \gamma_{07}\bigl(\text{nse\_cmc}\,\times\,\text{idh\_gmc}\bigr) \\[4pt]
    &+ u_{0j}
    + u_{1j}\,\text{nse\_cmc}
    + r_{ij}
      \end{aligned}
...
```{r Des1, echo=FALSE, message = FALSE,  fig.cap="Figura 1.3: Scatterplot relación entre promedio comunal de escolaridad y tasa de empleo", results='asis'}
casen_educ %>% 
  group_by(comuna) %>% 
  summarise(
...
```{r Des2, echo=FALSE, message = FALSE, fig.cap="Figura 1.4: Scatterplot: relación entre promedio comunal de escolaridad y IDH", results='asis'}
casen_educ %>% 
  group_by(comuna) %>% 
  summarise(
    esc_prom    = mean(esc, na.rm = TRUE),
...
m5_H6 <- lmer(
  esc ~ 1 +
    ...
    prop_pob_esc_gmc +  idh_gmc +                    # efecto principal contextual
    female +
    (1 + nse_cmc | comuna),
  data = casen_educ
)
...
# ── Test de devianza m3 vs m4 (pendiente aleatoria NSE) ───────────────
lr   <- anova(m3, m4)
...
foot <- sprintf(
  "<b>Test de devianza (pendiente aleatoria NSE):</b> Δ = %.2f (df = %d), p %s",
  delta, df_lr, p_txt
)
...
# Generar tabla con sjPlot -----------------------------------------------
tab_obj <- tab_model(
  m0, m1, m2, m3, m4, m5_H6, m6_H7,
  show.ci = FALSE, show.se = TRUE, p.style = "stars",
  digits  = 3,
  dv.labels = c(
    "Nulo (ICC)", "M1 (centrado)", "M2 (centrado)",
    "M1 + M2", "+ Pend. NSE", "+ Interacción H7", "+ Interacción H8"
  )
)

# HTML con estilos inline; mantiene formato sin cerrar <body> -------------
html_tab <- tab_obj$knitr   # <table> … </table> con estilos inline

# Nota del test de devianza -----------------------------------------------
nota <- sprintf(
  '<p class="mt-2 mb-4 small text-muted">
     <strong>Test de devianza</strong>
     (m3&nbsp;vs&nbsp;m4, pendiente aleatoria NSE):<br>
     &Delta;&nbsp;=&nbsp;%.2f&nbsp;(df&nbsp;=&nbsp;%d),&nbsp;p&nbsp;%s
   </p>',
  delta, df_lr, p_txt
)

# Devolvemos el HTML sin que Quarto lo envuelva en <code> ------------------
knitr::asis_output(
  paste0(
    '<style>.scroll-table{overflow-x:auto;width:100%;}</style>',
    '<div class="scroll-table">', html_tab, '</div>',
    nota
  )
)
...
```{r modelo_tabla1, echo=FALSE, fig.cap="Figura 1.6: Interacción entre NSE individual y tasa de empleo comunal sobre los años esperados de escolaridad ", warning=FALSE, results='asis'}
p_h6 <- plot_model(
  m5_H6,
  type  = "int",
...
```{r modelo_tabla2, echo=FALSE,  message=FALSE, fig.cap="Figura 1.7: Interacción entre NSE individual e Índice de Desarrollo Humano (IDH) comunal sobre los años esperados de escolaridad", warning=FALSE, results='asis'}
...
```{r r_fig_escolaridad, fig.cap="Figura 1.9: Modelo teórico con resultados de las hipótesis planteadas.", echo=FALSE}
...
```{r Anexo 1, echo=FALSE, fig.cap="Anexo 1: Tabla comparativa de estimación de modelos con y sin casos influyentes", results='asis'}
